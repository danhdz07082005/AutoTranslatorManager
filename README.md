...
- 🚀 **Zero-Config Game Detector**: Tự động nhận diện cấu trúc file của game để xác định engine chính xác (Unity Mono, Unity IL2CPP, Ren'Py, RPG Maker XP/VX/VX Ace/MV/MZ).
...
| **Ren'Py** | Dịch dịch ngược AST / bytecode (`unrpyc`), patch file `.rpy` | Ngoại tuyến (Offline Batch) | Kịch bản Ren'Py (`game/tl/{lang}/*.rpy`) | ✅ Có |
| **RPG Maker (MV / MZ)** | Phân tích cú pháp JSON `data/*.json`, vá overlay | Ngoại tuyến & Runtime Overlay | `www/data/*.json` hoặc Plugin patch | ✅ Có |
| **RPG Maker (XP / VX / VX Ace)** | Trích xuất Marshal Ruby data (`Scripts.rvdata2`, `Map*.rxdata`) | Ngoại tuyến (Offline Batch) | `Data/*.rvdata2` | ✅ Có |
...
2. 🔒 **Game bị mã hóa / Bảo vệ bản quyền (DRM / Anti-Tamper)**:
...
3. 🖼️ **Chữ vẽ cứng trên hình ảnh (Hardcoded Bitmap Text)**: ATM chỉ trích xuất và dịch chuỗi ký tự (text strings). Chữ được vẽ trực tiếp lên texture/sprite đồ họa cần dùng phần mềm chỉnh sửa ảnh hoặc công cụ texture patcher riêng.
4. 🌐 **Giới hạn API Dịch thuật (Rate Limits)**:
   - Khi dịch tự động số lượng lớn văn bản (hơn 10.000 câu) bằng các engine miễn phí (Google Translate, Bing), dịch vụ dịch có thể trả về lỗi `HTTP 429 Too Many Requests`.
...
5. 💻 **Hệ điều hành**: Thiết kế tối ưu cho **Windows 10 / Windows 11 (64-bit)**. Chưa hỗ trợ Linux / macOS native (có thể chạy qua Wine/Proton với cấu hình môi trường bổ sung).
...
flowchart TB
    ...
    subgraph Server_Layer ["Local API & Dispatcher Server"]
        HTTP_Server["FastAPI / WSGI Local Server (Port ngẫu nhiên/an toàn)"]
    ...
    subgraph Core_Pipeline ["Bộ Xử Lý Dịch Thuật (Core Translation Pipeline)"]
        ...
        Translators["Translation Providers (Google, DeepL, Offline AI)"]
...
### 🌟 Key Highlights:
- 🚀 **Zero-Config Engine Detection**: Automatically detects Unity Mono, Unity IL2CPP, Ren'Py, and RPG Maker (XP, VX, VX Ace, MV, MZ) game structures.
...
| **Unity (IL2CPP)** | Runtime injection via BepInEx 6 & Il2CppDumper | Real-time hooking | Text Dump (`_AutoGeneratedTranslations.txt`) | ✅ Yes |
| **Ren'Py** | Bytecode decompilation via `unrpyc` & AST string replacement | Offline batch | Ren'Py Scripts (`game/tl/{lang}/*.rpy`) | ✅ Yes |
| **RPG Maker (MV / MZ)** | JSON AST parsing & overlay patching | Offline & Runtime Overlay | `www/data/*.json` | ✅ Yes |
| **RPG Maker (XP / VX / VX Ace)** | Ruby Marshal deserialization & string substitution | Offline batch | `Data/*.rvdata2` | ✅ Yes |
...
2. 🔒 **Encrypted & Protected Assemblies (DRM / Anti-Cheat)**:
...
3. 🖼️ **Baked Bitmap Graphics / UI Text**: ATM translates textual strings extracted from scripts or runtime string tables. Hardcoded bitmap textures require separate graphic editing.
4. 🌐 **Machine Translation Rate Limits**:
   - Translating tens of thousands of lines using free translation tiers (Google, Bing) may trigger `HTTP 429 (Rate Limit)` responses.
...
5. 💻 **Operating System**: Engineered specifically for **Windows 10 / Windows 11 (64-bit)**.
...
```mermaid
graph TD
    Client["Web UI (Vanilla JS + CSS Variables)"] <--> Server["Local WSGI/HTTP Server & API"]
    Server <--> Manager["Translation Job Manager"]
    Manager <--> Translators["Translation Providers"]
    Manager <--> Storage[("SQLite WAL Storage")]
    Manager <--> Deployer["Engine Deployer (BepInEx / unrpyc)"]
