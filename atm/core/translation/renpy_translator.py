...
class RenPyTranslator:
    ...
    def _inject_language_config(game_path: Path, language: str) -> None:
        ...
        content = (
            ...
            f"    if getattr(persistent, '_atm_active_lang', None) != {language!r}:\n"
            f"        persistent._atm_active_lang = {language!r}\n"
            f"        persistent._atm_lang_initialized = True\n"
            f"        _preferences.language = {language!r}\n"
        )
