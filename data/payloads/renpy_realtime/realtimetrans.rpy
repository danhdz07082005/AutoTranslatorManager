init -999 python:
    ...
    def _send_batch_translation_request(texts, target_lang, source_lang="auto"):
        import json
        import urllib2
        try:
            url = "http://127.0.0.1:__ATM_SERVER_PORT__/api/translate"
            headers = {"Content-Type": "application/json"}
            data = json.dumps({"game_id": "__ATM_GAME_ID__", "texts": texts})
            req = urllib2.Request(url, data, headers)
            response = urllib2.urlopen(req, timeout=60)
            res_data = json.loads(response.read())
            if res_data.get("status") == "success":
                return res_data.get("translated_texts", texts)
        except Exception as e:
            print("ATM Translation Error: " + str(e))
        return texts
    ...
    def translate_batch(texts, target_lang, translation_service=persistent.translation_service):
        if (not persistent.enable_translation) or (not texts):
            return texts
        try:
            source_lang = persistent.source_lang if hasattr(persistent, 'source_lang') else "auto"
            return _send_batch_translation_request(texts, target_lang, source_lang)
        except Exception as e:
